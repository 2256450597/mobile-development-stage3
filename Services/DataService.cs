...
public class DataService : IDataService
{
    ...
    /// <summary>Returns all recipes in the data store.</summary>
    public List<Recipe> GetAllRecipes() => _recipes;

    /// <summary>Filters recipes by the specified food category.</summary>
    /// <param name="category">The food category to filter by.</param>
    /// <returns>A list of recipes matching the given category.</returns>
    public List<Recipe> GetRecipesByCategory(FoodCategory category)
        => _recipes.Where(r => r.Category == category).ToList();

    /// <summary>Looks up a single recipe by its unique identifier.</summary>
    /// <param name="id">The recipe ID to find.</param>
    /// <returns>The matching recipe, or null if not found.</returns>
    public Recipe? GetRecipeById(string id)
        => _recipes.FirstOrDefault(r => r.Id == id);

    /// <summary>Retrieves all meal plan entries falling within the 7-day week starting from the given Monday.</summary>
    /// <param name="weekStart">A DateTime representing the Monday of the target week.</param>
    /// <returns>List of meal plan entries for that week.</returns>
    ...
    /// <summary>Returns the full shopping list.</summary>
    public List<ShoppingItem> GetShoppingList() => _shoppingList;

    /// <summary>Adds a new entry to the weekly meal plan.</summary>
    /// <param name="entry">The meal plan entry to add.</param>
    ...
    /// <param name="item">The shopping item to add.</param>
    public void AddShoppingItem(ShoppingItem item) => _shoppingList.Add(item);

    /// <summary>Toggles the checked state of a shopping list item.</summary>
    ...
    /// <summary>Removes all auto-generated shopping list items.</summary>
    public void RemoveAutoGeneratedItems()
        => _shoppingList.RemoveAll(i => i.IsAutoGenerated);

    /// <summary>Adds a new recipe to the data store.</summary>
    /// <param name="recipe">The recipe to add.</param>
    public void AddRecipe(Recipe recipe) => _recipes.Add(recipe);

    /// <summary>Searches recipes by name, category, or ingredient using a case-insensitive substring match.</summary>
    /// <param name="query">The search query. If null or whitespace, all recipes are returned.</param>
    ...
    /// <summary>Creates 15 sample recipes across 3 cooking-method categories with ingredients, instructions, and nutritional data.</summary>
    private static List<Recipe> InitializeRecipes() => new()
    {
        new Recipe
        {
            ...
            Category = FoodCategory.Stovetop,
